...
CRYPTREST_INSTALLER_WWW_STRUCTURE_DIR="$CRYPTREST_INSTALLER_WWW_DIR/structure"
CRYPTREST_INSTALLER_WWW_STRUCTURE_HTML_FILE="$CRYPTREST_INSTALLER_WWW_STRUCTURE_DIR/index.html"
...
cryptrest_init()
{
    ...
    chmod 700 "$CRYPTREST_INSTALLER_WWW_DIR" && \
    mkdir -p "$CRYPTREST_INSTALLER_WWW_STRUCTURE_DIR" && \
    chmod 700 "$CRYPTREST_INSTALLER_WWW_STRUCTURE_DIR" && \
...
cryptrest_local_install()
{
    ...
    cp "$CRYPTREST_MAIN_LIBS_DIR/bin.sh" "$CRYPTREST_INSTALLER_WWW_STRUCTURE_HTML_FILE" && \
    return 0
}
...
cryptrest_network_install()
{
    ...
    cryptrest_download && \
    cp "$CRYPTREST_INSTALLER_LIB_FILE" "$CRYPTREST_INSTALLER_WWW_STRUCTURE_HTML_FILE" && \
...
{
    cryptrest_bin_installer_define && \
    chmod 444 "$CRYPTREST_INSTALLER_WWW_STRUCTURE_HTML_FILE" && \
    ...
    ln -s "$CRYPTREST__COMMON_WWW_HTML_DIR/i.html" "$CRYPTREST_INSTALLER_WWW_DIR/index.html" && \
...
{
    echo "CRYPTREST_LIB_DOMAIN=\"$CRYPTREST_INSTALLER_NAME.\$CRYPTREST_DOMAIN\"" > "$CRYPTREST_ETC_DOMAINS_DIR/$CRYPTREST_INSTALLER_NAME"
    echo "CRYPTREST_DOMAINS=\"$CRYPTREST_INSTALLER_NAME.\$CRYPTREST_DOMAIN\"" >> "$CRYPTREST_ETC_DOMAINS_DIR/$CRYPTREST_INSTALLER_NAME"
}
